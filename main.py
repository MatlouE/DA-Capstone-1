...
#build dashboard
add_sidebar = st.sidebar.selectbox('Aggregate or Individual Video', ('Agreegate Metrics', 'Individual Video Analysis'))

##total picture
if add_sidebar == 'Aggregate Metrics':
    df_agg_metrics = df_agg[['Video publish time', 'Views', 'Likes', 'Subscribers', 'Shares', 'Comments added', 'RPM(USD)','Average % Views', 'Avg_duration_sec', 'Engagement_ratio', 'Views /sub gained']]
    metric_date_6mo = df_agg_metrics['Video publishh time'].max()- pd.DateOffset(months=6)#most recent 6month date range
    ...
    df_agg_diff_final = df_agg_diff.iloc[:, ['Video publish time', 'Views', 'Likes', 'Subscribers', 'Shares', 'Comments added', 'RPM(USD)','Average % Views', 'Avg_duration_sec', 'Engagement_ratio', 'Views /sub gained']]
    
    st.dataframe(df=df_agg_diff_final)
